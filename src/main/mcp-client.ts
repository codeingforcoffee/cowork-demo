...
function mcpInputSchemaToParameters(inputSchema: {
  ...
  return {
    ...
    properties: (inputSchema.properties as Record<string, { type?: string; description?: string }>) || {},
...
export async function connectMcpServer(server: McpServer): Promise<McpConnection | null> {
  ...
  async function doConnect(
    ...
    try {
      ...
      console.log('[mcp-client] connected:', server.name, 'tools:', tools.map((x) => x.function.name))
