...
const canSend = computed(() => input.value.trim().length > 0 && !sessionsStore.isStreaming);
...
function handleClickOutside(e: MouseEvent): void {
  ...
  if (!target.closest('.expert-panel-container')) {
...
const currentExpert = computed(() =>
  sessionsStore.currentExpertId ? expertsStore.getExpertById(sessionsStore.currentExpertId) : null
);
...
<template>
  <div class="flex flex-col h-full" @click="handleClickOutside">
    <!-- Header -->
    <header class="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-border">
      <!-- Session title -->
      <h1 class="text-sm font-semibold text-text-primary truncate flex-1">
        {{ sessionsStore.currentSession?.title || 'New Chat' }}
      </h1>

      <!-- Expert panel trigger -->
      <div class="relative expert-panel-container">
        <button
          ...
          :class="currentExpert ? 'border-accent/50 text-accent' : ''"
          :title="t('experts.selectExpert')"
          @click.stop="showExpertPanel = !showExpertPanel"
        >
          <span class="text-sm">🧠</span>
          {{ t('experts.expert') }}
          <span v-if="currentExpert" class="truncate max-w-16">{{ currentExpert.name }}</span>
        </button>
        <div
          v-if="showExpertPanel"
          class="absolute right-0 top-full mt-1 w-56 bg-bg-secondary border border-border rounded-xl shadow-lg z-50 p-3"
          @click.stop
        >
          <p class="text-xs font-medium text-text-secondary mb-2">
            {{ t('experts.selectExpert') }}
          </p>
          <button
            class="w-full flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs transition-colors mb-1"
            :class="
              !sessionsStore.currentExpertId
                ? 'bg-accent/15 text-accent'
                : 'text-text-secondary hover:bg-bg-tertiary hover:text-text-primary'
            "
            @click="
              sessionsStore.setExpert(undefined);
          ...
          >
            <button
              ...
              :class="
                sessionsStore.currentExpertId === expert.id
                  ? 'bg-accent/15 text-accent'
                  : 'text-text-secondary hover:bg-bg-tertiary hover:text-text-primary'
              "
              @click="
                sessionsStore.setExpert(expert.id);
                showExpertPanel = false;
              "
            >
              <span>🧠</span>
          ...
          <p
            ...
            class="text-xs text-text-tertiary py-2 text-center"
          >
            {{ t('experts.noExpertsGoCreate') }}
          </p>
        </div>
      </div>
      ...
      <div class="relative mcp-panel-container">
        <button
          class="flex items-center gap-1.5 rounded-lg border border-border px-2.5 py-1.5 text-xs text-text-secondary hover:text-text-primary hover:bg-bg-tertiary transition-colors"
          :class="
            sessionsStore.currentEnabledMcpIds.length > 0 ? 'border-accent/50 text-accent' : ''
          "
        ...
        >
          ...
          <div
            v-if="mcpStore.allServers.length === 0"
            class="text-xs text-text-tertiary py-2 text-center"
          >
          ...
          <div
            v-for="server in mcpStore.allServers"
            :key="server.id"
            class="flex items-center gap-2 py-1.5"
          >
            <button
              ...
              :class="
                sessionsStore.currentEnabledMcpIds.includes(server.id)
                  ? 'bg-accent'
                  : 'bg-bg-tertiary'
              "
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0">
      ...
      >
        <svg width="40" height="40" viewBox="0 0 40 40" fill="none" class="mb-4 opacity-30">
          <path
            d="M20 4C11.16 4 4 11.16 4 20c0 2.96.79 5.74 2.17 8.13L4 36l7.87-2.17A15.93 15.93 0 0020 36c8.84 0 16-7.16 16-16S28.84 4 20 4z"
            stroke="currentColor"
            stroke-width="2"
      ...
      <template v-for="msg in sessionsStore.currentMessages" :key="msg.id">
        <!-- Tool call indicator -->
        <div v-if="msg.toolCallName" class="flex items-start gap-2 mr-8">
          <div
            class="rounded-xl bg-bg-tertiary border border-border px-3 py-2 text-xs text-text-secondary w-full"
          >
            <div class="flex items-center gap-1.5 mb-1">
              ...
              <span class="font-medium text-accent">
                ...
                <template v-else-if="msg.toolCallName === 'get_system_info'"
            ...
            <div
              v-if="msg.toolResultContent"
              class="mt-1.5 pt-1.5 border-t border-border text-[10px] text-text-secondary whitespace-pre-wrap max-h-32 overflow-y-auto"
            >
      ...
      <div
        ...
        class="rounded-lg bg-red-500/15 border border-red-500/40 p-3 text-sm text-red-600 dark:text-red-400"
      >
        {{ sessionsStore.error }}
      </div>
