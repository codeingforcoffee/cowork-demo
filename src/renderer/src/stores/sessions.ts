...
export const useSessionsStore = defineStore('sessions', () => {
  ...
  async function init(): Promise<void> {
    ...
    try {
      ...
      if (data.lastActiveSessionId && sessions.value.find(s => s.id === data.lastActiveSessionId)) {
  ...
  async function sendMessage(
    ...
    try {
      const apiMessages = buildApiMessages(session.id)
      // Exclude the last empty assistant placeholder
      const messagesToSend = apiMessages.slice(0, -1)
      // session.enabledMcpIds 是 Vue Proxy 数组，需展开为普通数组再传 IPC
      await window.api.sendChat(messagesToSend, [...session.enabledMcpIds])
    } catch (err) {
