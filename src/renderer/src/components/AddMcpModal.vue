      ...
      if (props.editingServer) {
        ...
        command.value =
          props.editingServer.type === 'stdio'
            ? [props.editingServer.command, ...(props.editingServer.args || [])]
                .filter(Boolean)
                .join(' ')
            : '';
      } else {
  ...
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div class="space-y-4 px-6 py-5">
          ...
          <div v-if="serverType === 'local'">
            ...
            <p class="mt-1 text-xs text-text-tertiary">
              输入完整命令，参数用空格分隔，例如：npx -y @modelcontextprotocol/server-filesystem
              /path
            </p>
          </div>
        ...
        <!-- Footer -->
        <div
          class="flex items-center justify-end gap-3 border-t border-border bg-bg-secondary/50 px-6 py-4"
        >
          <button
            type="button"
